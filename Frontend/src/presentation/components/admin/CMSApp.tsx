...
export function CMSApp() {
  ...
  return (
    <div className="flex h-screen bg-[#030014] text-gray-200">
      {/* Sidebar */}
      <aside className="w-64 glass-panel border-r border-white/10 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button
            type="button"
            onClick={() => { setActiveTab('articles'); fetchItems(); }}
          ...
          <button
            type="button"
            onClick={() => { setActiveTab('projects'); fetchItems(); }}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
              activeTab === 'projects'
                ? 'bg-brand-500/20 text-brand-300 border border-brand-500/30'
                : 'hover:bg-white/5 text-gray-400'
          ...
          {loading && items.length === 0 ? (
            ...
            <div className="grid gap-4">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="glass-panel p-6 flex items-center justify-between border border-white/10 hover:border-brand-500/30 transition-colors"
                >
                  <div>
                    ...
                    <div className="flex gap-3 mt-2">
                      {item.status && (
                        <span
                          className={`text-xs px-2 py-1 rounded border ${item.status === 'published' ? 'border-green-500/30 text-green-400 bg-green-500/10' : 'border-yellow-500/30 text-yellow-400 bg-yellow-500/10'}`}
                        >
                          {item.status}
                        </span>
                      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-2xl max-h-[90vh] overflow-auto border border-brand-500/30 shadow-2xl">
            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-[#0A0A1F]/50 sticky top-0 z-10">
              <h3 className="text-xl font-bold">
                {editingItem ? 'Edit' : 'Create'} {activeTab === 'articles' ? 'Article' : 'Project'}
              </h3>
              <button
                type="button"
